...
For the next phase we want to integrate local inference, putting my new DGX Spark to a good use, but our architecture was hardcoded to an Anthropic CLI wrapper. Furthermore, we wanted to experiment with OpenRouter's vast model marketplace and Vertex AI's enterprise endpoints.
