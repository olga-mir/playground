...
- For auth errors: run `kubectl describe gitrepository <name> -n <ns> --context <ctx>` and include the verbatim `.status.conditions[].message` in the errors list — the exact wording (e.g. `provider is not set to github`, `has github app data`) is how a downstream diagnostics agent recognizes known issues. Summarising the message loses that signal.
