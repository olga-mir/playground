...
#
# Workload Identity for Flux image scanning of Artifact Registry (apps-dev only)
#

gcloud iam service-accounts create flux-image-reader-sa \
    --project=$PROJECT_ID \
    --display-name="Flux Image Reflector - Artifact Registry Reader"

# Scoped to the experiments repo only, not project-wide.
gcloud artifacts repositories add-iam-policy-binding experiments \
    --project=$PROJECT_ID \
    --location=$REGION \
    --member="serviceAccount:flux-image-reader-sa@${PROJECT_ID}.iam.gserviceaccount.com" \
    --role="roles/artifactregistry.reader"

# Workload Identity binding: lets image-reflector-controller on apps-dev impersonate
# flux-image-reader-sa without a key file. KSA is annotated via the apps-dev FluxInstance's
# spec.kustomize.patches (see kubernetes/clusters/apps-dev/flux-system/flux-instance.yaml).
gcloud iam service-accounts add-iam-policy-binding \
    flux-image-reader-sa@${PROJECT_ID}.iam.gserviceaccount.com \
    --project=$PROJECT_ID \
